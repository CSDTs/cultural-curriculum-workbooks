...
const WORKBOOK_API_HOST = "http://127.0.0.1:8000/api/workbook_saves/";
const CLASSROOM_API_HOST = "http://127.0.0.1:8000/api/classroom/";
const USER_API_HOST = "http://127.0.0.1:8000/api/users/c";
const USER_CLASSROOMS_API_HOST = "http://127.0.0.1:8000/api/team/";

let _csrfToken = null;

async function getCsrfToken() {
	if (_csrfToken === null) {
		const response = await fetch(`${WORKBOOK_API_HOST}/csrf/`, {
			credentials: "include",
		});
		const data = await response.json();
...
async function workbookRequest(project, method) {
	...
	if (method === "PATCH") query = `${project?.id || ""}`;
...
export { getCsrfToken, workbookRequest, getClassroomInfo, getClassrooms, getUser };
